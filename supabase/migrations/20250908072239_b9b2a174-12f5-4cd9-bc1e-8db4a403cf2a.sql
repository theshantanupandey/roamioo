-- Add trending places data to the database (final version)
INSERT INTO places (
...
-- Update popularity scores and rankings using a CTE
WITH ranked_places AS (
  SELECT 
    id,
    (average_rating * total_reviews) + (total_reviews * 0.1) AS pop_score,
    ROW_NUMBER() OVER (ORDER BY (average_rating * total_reviews) + (total_reviews * 0.1) DESC) AS rank
  FROM places
  WHERE is_verified = true
)
UPDATE places 
SET 
  popularity_score = ranked_places.pop_score,
  trending_rank = ranked_places.rank
FROM ranked_places
WHERE places.id = ranked_places.id;